...
def init_test_data():
    ...
    print( type( edges_weight ) )
    print( type( y ) )
    return x.cuda(), edge_index.cuda(), edges_weight.cuda(), y.cuda() , nclass, nfeat
...
def init_data( train_rate ):
    ...
    return torch.tensor( x, dtype = torch.float32 ).cuda(), torch.tensor( edge_index ).cuda(), np.array( edges_weight ), torch.tensor( y ).cuda(), len( cate_dict ), nfeat, cate_dict, train_mask, val_mask, test_mask, nnodes
...
def main( train_rate, nepoch ):
    ...
    graph = data.Data( x = x, edge_index = edge_index, edges_weight = edges_weight, y = y, train_mask = train_mask, val_mask = val_mask, test_mask = test_mask ).cuda()
    print( "Graph build done..." )

    print( "Begin to build GCN..." )
    model = GCN( nfeat = nfeat, nhid = 128, nclass = nclass ).cuda()
