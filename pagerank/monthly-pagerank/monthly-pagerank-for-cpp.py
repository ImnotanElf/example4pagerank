            ...
            with open( txt_list[ j ], 'r' ) as fr:
                ...
                for line in lines:
                    ...
                    if tx_from not in ad2id_d:
                        ad2id_d[ tx_from ] = ad_count
                        ad_count += 1
                    edges.append( [ ad2id_d[ tx_from ], ad2id_d[ tx_to ] ] )
        print( i, "th number of edges: ", len( edges ) )
        print( i, "th number of addrs: ", len( ad2id_d ) )
        print( i, "th count of loop: ", count )
        with open( f"./outs/{ start_datetime.replace( ' ', '-' ) }---{ end_datetime.replace( ' ', '-' ) }-dict-cpp.txt", 'w' ) as fw:
            for k, v in ad2id_d.items():
                fw.write( str( k ) + ',' + str( v ) + '\n' )
        ad2id_d.clear()
        with open( f"./outs/{ start_datetime.replace( ' ', '-' ) }---{ end_datetime.replace( ' ', '-' ) }-edges-cpp.txt", 'w' ) as fw:
            for edge in edges:
