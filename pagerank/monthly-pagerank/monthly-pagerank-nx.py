...
def get_txt_list():
    ...
    with open( './cpp/data/txt_list.txt', 'w' ) as fw:
        for txt in txt_list:
            fw.write( txt + '\n' )
    return txt_list
...
def process():
    ...
    for i in range( 0, 8 ):
        ...
        with open( f"./outs/{ start_datetime.replace( ' ', '-' ) }---{ end_datetime.replace( ' ', '-' ) }-dict-nx.txt", 'w' ) as fw:
        ...
        pr = nx.pagerank(G1, alpha=0.85, personalization=None,
                ...
                dangling=None)
        pr = sorted( pr.items(), key=lambda s:s[1] )
        for i in range( 10 ):
            print( pr[-1 - i] )

        print( i, "th Number of vertices:", len( pr ) )
        with open( f"./outs/{ start_datetime.replace( ' ', '-' ) }---{ end_datetime.replace( ' ', '-' ) }-nx.txt", 'w' ) as fw:
            for i in range( len( pr ) ):
                fw.write( str( pr[ i ][ 0 ] ) + ',' + str( pr[ i ][ 1 ] ) + '\n' )
        with open( f"./outs/{ start_stamp }-{ end_stamp }-nx.txt", 'w' ) as fw:
            for i in range( len( pr ) ):
                fw.write( str( pr[ i ][ 0 ] ) + ',' + str( pr[ i ][ 1 ] ) + '\n' )
        G1.clear()
